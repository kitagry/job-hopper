...
const CommandsForm: FC<CommandsFormProps> = ({
  control,
  containerIndex,
  fieldName,
}) => {
  const { fields, append, remove } = useFieldArray({
    control,
    name: `spec.containers.${containerIndex}.${fieldName}` as `spec.containers.0.command`,
  });
        ...
        <Grid item xs={12} key={command.id}>
          <Grid container>
            <Grid item xs={10}>
              <Controller
                name={
                  `spec.containers.${containerIndex}.${fieldName}.${index}.value` as const
                }
                control={control}
                rules={{ required: true }}
